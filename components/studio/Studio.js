...
function Workspace({ installPrompt, onInstalled }) {
  ...
  const commands = [
    ...
    { id: 'v-live', group: 'View', icon: '🎤', label: 'Go to Live inputs', keywords: 'mic turntable line record channel', run: () => setUi({ view: 'liveinputs' }) },
        ...
        <nav className={s.bottomNav}>
          ...
          {TABS.map((t) => (
            ...
            >
              ...
              {({ playlist: 'Arrange', rack: 'Instr', piano: 'Piano', drums: 'Drums', mixer: 'Mixer', automation: 'Auto', mastering: 'Master', video: 'Video', liveinputs: 'Inputs', workspace: 'Custom' })[t.id]}
            </button>
          ))}
        </nav>
      )}
