...
export function makePattern(name, bars = 1, color = COLORS[3]) {
  return { id: uid('p'), name, bars, color, notes: {} };
}
...
export function createDefaultProject() {
  ...
  const kick = makeChannel('kick', 'Kick', { color: COLORS[0], insert: inserts[0].id, vol: 0.95 });
  const clap = makeChannel('clap', 'Clap', { color: COLORS[2], insert: inserts[1].id, vol: 0.6 });
  const hat = makeChannel('hat', 'Hat', { color: COLORS[3], insert: inserts[2].id, vol: 0.4 });
  const openhat = makeChannel('hat', 'Open Hat', {
    color: COLORS[10], insert: inserts[2].id, vol: 0.28, params: { decay: 0.34, hpf: 8200 },
  });
  const snare = makeChannel('snare', 'Snare', { color: COLORS[1], insert: inserts[3].id, vol: 0.5 });
...
export function createEmptyProject() {
  ...
  for (let i = 1; i <= 8; i++) inserts.push(makeInsert(i));
  const channels = [
    makeChannel('kick', 'Kick', { color: COLORS[0], insert: inserts[0].id, vol: 0.95 }),
    makeChannel('clap', 'Clap', { color: COLORS[2], insert: inserts[1].id, vol: 0.6 }),
    makeChannel('hat', 'Hat', { color: COLORS[3], insert: inserts[2].id, vol: 0.4 }),
    makeChannel('snare', 'Snare', { color: COLORS[1], insert: inserts[3].id, vol: 0.55 }),
  ];
...
export function normalizeProject(raw) {
  ...
  p.patterns = p.patterns.map((pt) => ({ ...makePattern(pt.name || 'Pattern'), ...pt, notes: pt.notes || {} }));
