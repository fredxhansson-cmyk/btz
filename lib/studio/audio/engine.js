...
export class Engine {
  ...
  triggerNote(channel, note, time) {
    const out = this.graph.channelInput(channel.id);
    const dur = Math.max(0.01, note.d * this.secondsPerTick);
    const voice = playNote(this.ctx, out, channel, {
  ...
  preview(channelId, key = 60, vel = 0.9, dur = 0.5) {
    ...
    if (!ch) return;
    const time = ctx.currentTime + 0.005;
    const voice = playNote(ctx, this.graph.channelInput(ch.id), ch, { time, key, vel, dur }, this.buffers);
    if (voice && voice.release) voice.release(time + dur);
  }
