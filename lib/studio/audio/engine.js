...
export class Engine {
  ...
  setProject(project) {
    ...
    if (this.playing && this.mode === 'song') this.rebuildMap();
  ...
  rebuildMap() {
    ...
    this.glideMap = this.buildGlideMap(p);
  }

  /**
   * noteId -> previous note's key, for channels with Glide on, when notes are
   * legato/overlapping. Drives 808/bass pitch slides. Same for pattern & song
   * playback since it is keyed by note id within the patterns.
   */
  buildGlideMap(p) {
    const gm = new Map();
    const GAP = 3; // ticks of tolerance so near-touching notes still slide
    for (const ch of p.channels) {
      const glide = ch.params && ch.params.glide;
      if (!(glide > 0)) continue;
      for (const pat of p.patterns) {
        const notes = (pat.notes && pat.notes[ch.id]) || [];
        if (notes.length < 2) continue;
        const sorted = [...notes].sort((a, b) => a.t - b.t);
        for (let i = 1; i < sorted.length; i++) {
          const prev = sorted[i - 1];
          const cur = sorted[i];
          if (prev.k !== cur.k && prev.t + prev.d >= cur.t - GAP) gm.set(cur.id, prev.k);
        }
      }
    }
    return gm;
  ...
  triggerNote(channel, note, time) {
    const glideFrom = this.glideMap ? this.glideMap.get(note.id) : undefined;
    triggerVoice(this.ctx, this.graph, this.resolveChannel(channel), note, time, this.secondsPerTick, this.buffers, this.chokes, glideFrom);
  ...
  /** Audition a bundled factory sample (library kind:'sample') by URL. Fetches
      + decodes once, then plays the one-shot. */
  async previewSampleUrl(url) {
    const ctx = this.ensureContext();
    if (!ctx) return;
    if (!this._sampleCache) this._sampleCache = {};
    let buf = this._sampleCache[url];
    if (!buf) {
      try {
        const r = await fetch(url);
        buf = await ctx.decodeAudioData(await r.arrayBuffer());
        this._sampleCache[url] = buf;
      } catch (e) { return; }
    }
    ...
    g.gain.value = 0.9;
    src.connect(g);
    g.connect(this.masterNode || ctx.destination);
    src.start();
  }
  ...
  /** Destination node for the live input: a mixer insert (own fader/FX/pan)
      when an insertId is given, otherwise straight into the master bus. */
  _inputDest(insertId) {
    if (this.graph) {
      if (insertId && insertId !== 'master') {
        try {
          const node = this.graph.ensureInsert({ id: insertId });
          if (node && node.input) return node.input;
        } catch (e) { /* fall through to master */ }
      }
      return this.graph.masterIn;
    }
    return this.ctx.destination;
  }

  /** Re-route a live input to a mixer channel (or 'master') without reopening. */
  setInputRoute(insertId) {
    if (!this.input) return;
    try { this.input.monitor.disconnect(); } catch (e) { /* noop */ }
    this.input.route = insertId || null;
    this.input.monitor.connect(this._inputDest(insertId));
  }

  /** Opens a microphone or line input and wires it up for monitoring. */
  async openInput(deviceId, insertId) {
    const ctx = this.ensureContext();
    ...
    monitor.connect(this._inputDest(insertId));
    this.input = { stream, source, gain, analyser, monitor, buf: new Uint8Array(1024), route: insertId || null };
    this.emit();
    return stream;
  }
  ...
  // Persistent, always-monitored inputs (mic, turntable, line…) each routed to
  // its own mixer channel. Separate from the single record input above so the
  // recording flow is never disturbed.

  /** Open an extra live input and route it to a mixer insert (or master). */
  async addLiveInput(deviceId, insertId, name) {
    const ctx = this.ensureContext();
    if (!ctx || typeof navigator === 'undefined' || !navigator.mediaDevices) {
      throw new Error('Audio input is not supported in this browser.');
    }
    if (!this.liveInputs) this.liveInputs = new Map();
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: {
        deviceId: deviceId ? { exact: deviceId } : undefined,
        echoCancellation: false,
    ...
    source.connect(gain);
    gain.connect(analyser);
    gain.connect(this._inputDest(insertId));
    this._liId = (this._liId || 0) + 1;
    const id = `li${this._liId}`;
    this.liveInputs.set(id, {
      id, stream, source, gain, analyser, buf: new Uint8Array(1024),
      route: insertId || null, deviceId: deviceId || '', name: name || 'Live input',
    });
    this.emit();
    return id;
  }
  ...
  removeLiveInput(id) {
    const li = this.liveInputs && this.liveInputs.get(id);
    if (!li) return;
    try { li.stream.getTracks().forEach((t) => t.stop()); } catch (e) { /* noop */ }
    try { li.gain.disconnect(); } catch (e) { /* noop */ }
    try { li.source.disconnect(); } catch (e) { /* noop */ }
    this.liveInputs.delete(id);
    this.emit();
  }

  setLiveInputGain(id, v) {
    const li = this.liveInputs && this.liveInputs.get(id);
    if (li) li.gain.gain.value = v;
  }

  setLiveInputRoute(id, insertId) {
    const li = this.liveInputs && this.liveInputs.get(id);
    if (!li) return;
    try { li.gain.disconnect(); } catch (e) { /* noop */ }
    li.route = insertId || null;
    li.gain.connect(li.analyser);
    li.gain.connect(this._inputDest(insertId));
  }

  liveInputLevel(id) {
    const li = this.liveInputs && this.liveInputs.get(id);
    if (!li) return 0;
    return levelFromAnalyser(li.analyser, li.buf);
  }

  get recording() { return !!(this.recorder && this.recorder.state === 'recording'); }

  startRecording() {
    if (!this.input) throw new Error('No input selected.');
    const MR = typeof window !== 'undefined' && window.MediaRecorder;
    if (!MR) throw new Error('Recording is not supported in this browser.');
  ...
  stopRecording() {
    return new Promise((resolve, reject) => {
      ...
      if (!rec || rec.state !== 'recording') { reject(new Error('No recording in progress.')); return; }
      rec.onstop = async () => {
        this.recorder = null;
        this.emit();
        try {
  ...
  /**
   * Awaits decoding of every sample in the project. Call before an offline
   * bounce so freshly-loaded sample instruments aren't rendered silent — the
   * realtime graph decodes lazily, which races against an immediate export.
   */
  async ensureSamplesReady(project) {
    if (!project || !project.samples) return;
    const ids = Object.keys(project.samples).filter((id) => project.samples[id] && project.samples[id].data);
    if (!ids.length) return;
    this.ensureContext();
    this.ensureSamples(project);
    const start = Date.now();
    while (ids.some((id) => !this.buffers[id]) && Date.now() - start < 10000) {
