// Offline bounce: renders the pattern or the full song to a WAV blob.
import { PPQ, STEP_TICKS } from '../constants';
import { MixerGraph } from './graph';
import { triggerNote } from './trigger';
import { buildPatternMap, buildSongMap, patternTicks, collectAutomation } from '../sequencer';
import { listTargets, denorm, AUTO_STEP } from '../automation';

export async function renderProject(project, buffers, opts = {}) {
  const mode = opts.mode || 'song';
  const repeats = Math.max(1, opts.repeats || 1);
  const tail = opts.tail == null ? 2.5 : opts.tail;
  const sampleRate = opts.sampleRate || 44100;
  const spt = 60 / Math.max(20, project.bpm) / PPQ;

  let map;
  let length;
  if (mode === 'song') {
    const song = buildSongMap(project);
    map = song.map;
    length = song.length;
  } else {
    map = buildPatternMap(project, project.activePattern);
    length = patternTicks(project.patterns.find((p) => p.id === project.activePattern));
  }
  if (!length) throw new Error('Nothing to render - add notes first.');

  const totalTicks = length * repeats;
  const duration = totalTicks * spt + tail;
  const OAC = typeof window !== 'undefined' && (window.OfflineAudioContext || window.webkitOfflineAudioContext);
  if (!OAC) throw new Error('OfflineAudioContext is not supported in this browser.');
  const ctx = new OAC(2, Math.ceil(sampleRate * duration), sampleRate);
  const graph = new MixerGraph(ctx, project, { analysers: false });
  graph.resyncAll(0, project);

  const anySolo = project.channels.some((c) => c.solo);
  const chokes = new Map();

  // Automation is rendered on the same grid the realtime engine uses.
  const { evals, targets } = collectAutomation(project, mode);
  const specs = new Map(listTargets(project).map((t) => [t.id, t]));
  graph.automated = targets;
  const paramAuto = new Map();
  const autoAt = (pos, time) => {
    for (const lane of evals) {
      const v = lane.at(pos);
      if (v == null) continue;
      const meta = specs.get(lane.target);
      if (!meta) continue;
      if (meta.perNote) {
        const parts = lane.target.split('|');
        const cur = paramAuto.get(parts[1]) || {};
        cur[parts[3]] = denorm(meta.spec, v);
        paramAuto.set(parts[1], cur);
      } else {
        graph.applyAutomation(project, lane.target, v, time, meta);
      }
    }
  };
  const resolve = (ch) => {
    const over = paramAuto.get(ch.id);
    return over ? { ...ch, params: { ...ch.params, ...over } } : ch;
  };
  const swing = project.swing || 0;
  const events = [];
  for (let r = 0; r < repeats; r++) {
    for (const [tick, list] of map) {
      for (const ev of list) events.push({ tick: tick + r * length, local: tick, ev });
    }
  }
  // Choke groups depend on order, so render strictly in time order.
  events.sort((a, b) => a.tick - b.tick);

  // 808/bass slides: noteId -> previous key on legato channels with Glide on.
  const glideMap = new Map();
  for (const ch of project.channels) {
    if (!(ch.params && ch.params.glide > 0)) continue;
    for (const pat of project.patterns) {
      const ns = (pat.notes && pat.notes[ch.id]) || [];
      if (ns.length < 2) continue;
      const sorted = [...ns].sort((a, b) => a.t - b.t);
      for (let i = 1; i < sorted.length; i++) {
        if (sorted[i - 1].k !== sorted[i].k && sorted[i - 1].t + sorted[i - 1].d >= sorted[i].t - 3) {
          glideMap.set(sorted[i].id, sorted[i - 1].k);
        }
      }
    }
  }
  let autoTick = 0;
  const advanceAuto = (untilTick) => {
    if (!evals.length) return;
    for (; autoTick <= untilTick; autoTick += AUTO_STEP) {
      const pos = mode === 'song' ? autoTick : autoTick % Math.max(1, length);
      autoAt(pos, autoTick * spt);
    }
  };
  for (const item of events) {
    advanceAuto(item.tick);
    const ch = project.channels.find((c) => c.id === item.ev.channelId);
    if (!ch || ch.mute || (anySolo && !ch.solo)) continue;
    let time = item.tick * spt;
    if (swing > 0 && item.local % (STEP_TICKS * 2) === STEP_TICKS) {
      time += swing * 0.5 * STEP_TICKS * spt;
    }
    triggerNote(ctx, graph, resolve(ch), item.ev.note, time, spt, buffers, chokes, glideMap.get(item.ev.note.id));
  }
  advanceAuto(totalTicks);

  // Progress via suspend/resume is expensive — every suspend pauses the render
  // and bounces to the event loop. It's only worth it for long bounces; short
  // patterns/songs render in one shot so export feels instant.
  if (typeof opts.onProgress === 'function' && ctx.suspend && duration > 6) {
    const STEPS = Math.min(6, Math.max(2, Math.round(duration / 6)));
    for (let i = 1; i < STEPS; i++) {
      const at = (duration * i) / STEPS;
      // eslint-disable-next-line no-loop-func
      ctx.suspend(at).then(() => {
        opts.onProgress(i / STEPS);
        ctx.resume();
      }).catch(() => {});
    }
  }

  return ctx.startRendering();
}

/** 16-bit PCM WAV encoder. */
export function encodeWav(buffer) {
  const channels = buffer.numberOfChannels;
  const frames = buffer.length;
  const bytes = frames * channels * 2;
  const view = new DataView(new ArrayBuffer(44 + bytes));
  const str = (off, s) => { for (let i = 0; i < s.length; i++) view.setUint8(off + i, s.charCodeAt(i)); };
  str(0, 'RIFF');
  view.setUint32(4, 36 + bytes, true);
  str(8, 'WAVE');
  str(12, 'fmt ');
  view.setUint32(16, 16, true);
  view.setUint16(20, 1, true);
  view.setUint16(22, channels, true);
  view.setUint32(24, buffer.sampleRate, true);
  view.setUint32(28, buffer.sampleRate * channels * 2, true);
  view.setUint16(32, channels * 2, true);
  view.setUint16(34, 16, true);
  str(36, 'data');
  view.setUint32(40, bytes, true);

  const data = [];
  for (let c = 0; c < channels; c++) data.push(buffer.getChannelData(c));
  let off = 44;
  for (let i = 0; i < frames; i++) {
    for (let c = 0; c < channels; c++) {
      const s = Math.max(-1, Math.min(1, data[c][i]));
      view.setInt16(off, s < 0 ? s * 0x8000 : s * 0x7fff, true);
      off += 2;
    }
  }
  return new Blob([view.buffer], { type: 'audio/wav' });
}

/** Writes a positive sample rate as an 80-bit IEEE-754 extended float (AIFF). */
function writeExtended(view, offset, rate) {
  if (!rate || rate <= 0) { for (let i = 0; i < 10; i++) view.setUint8(offset + i, 0); return; }
  const exponent = Math.floor(Math.log2(rate));
  const mantissa = rate / Math.pow(2, exponent); // in [1, 2)
  view.setUint16(offset, (exponent + 16383) & 0x7fff, false);
  const hi = Math.floor(mantissa * Math.pow(2, 31));      // top bit set (integer part)
  view.setUint32(offset + 2, hi >>> 0, false);
  const lo = Math.floor((mantissa * Math.pow(2, 31) - hi) * Math.pow(2, 32));
  view.setUint32(offset + 6, lo >>> 0, false);
}

/** 16-bit big-endian AIFF encoder. */
export function encodeAiff(buffer) {
  const channels = buffer.numberOfChannels;
  const frames = buffer.length;
  const dataBytes = frames * channels * 2;
  const view = new DataView(new ArrayBuffer(54 + dataBytes));
  let p = 0;
  const str = (s) => { for (let i = 0; i < s.length; i++) view.setUint8(p++, s.charCodeAt(i)); };
  const u32 = (v) => { view.setUint32(p, v, false); p += 4; };
  const u16 = (v) => { view.setUint16(p, v, false); p += 2; };
  str('FORM'); u32(46 + dataBytes); str('AIFF');
  str('COMM'); u32(18); u16(channels); u32(frames); u16(16);
  writeExtended(view, p, buffer.sampleRate); p += 10;
  str('SSND'); u32(8 + dataBytes); u32(0); u32(0);
  const data = [];
  for (let c = 0; c < channels; c++) data.push(buffer.getChannelData(c));
  for (let i = 0; i < frames; i++) {
    for (let c = 0; c < channels; c++) {
      const s = Math.max(-1, Math.min(1, data[c][i]));
      view.setInt16(p, s < 0 ? s * 0x8000 : s * 0x7fff, false); // big-endian
      p += 2;
    }
  }
  return new Blob([view.buffer], { type: 'audio/aiff' });
}

const toInt16 = (f32) => {
  const out = new Int16Array(f32.length);
  for (let i = 0; i < f32.length; i++) {
    const s = Math.max(-1, Math.min(1, f32[i]));
    out[i] = s < 0 ? s * 0x8000 : s * 0x7fff;
  }
  return out;
};

/** MP3 encoder (lamejs, loaded on demand so it stays out of the initial bundle). */
export async function encodeMp3(buffer, bitrate = 192) {
  const mod = await import('@breezystack/lamejs');
  const Encoder = mod.Mp3Encoder || (mod.default && mod.default.Mp3Encoder);
  const channels = Math.min(2, buffer.numberOfChannels);
  const enc = new Encoder(channels, buffer.sampleRate, bitrate);
  const left = toInt16(buffer.getChannelData(0));
  const right = channels > 1 ? toInt16(buffer.getChannelData(1)) : null;
  const block = 1152;
  const parts = [];
  for (let i = 0; i < left.length; i += block) {
    const l = left.subarray(i, i + block);
    const chunk = right ? enc.encodeBuffer(l, right.subarray(i, i + block)) : enc.encodeBuffer(l);
    if (chunk.length) parts.push(new Uint8Array(chunk));
  }
  const tail = enc.flush();
  if (tail.length) parts.push(new Uint8Array(tail));
  return new Blob(parts, { type: 'audio/mpeg' });
}

/** Encode a rendered buffer to the chosen container. */
export function encodeAudio(buffer, format = 'wav', opts = {}) {
  if (format === 'mp3') return encodeMp3(buffer, opts.bitrate || 192);
  if (format === 'aiff') return encodeAiff(buffer);
  return encodeWav(buffer); // sync; callers await uniformly
}

export function downloadBlob(blob, filename) {
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = filename;
  document.body.appendChild(a);
  a.click();
  setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 1000);
}

const MIME = {
  wav: 'audio/wav', mp3: 'audio/mpeg', aiff: 'audio/aiff', aif: 'audio/aiff',
  mid: 'audio/midi', midi: 'audio/midi', json: 'application/json',
};

/**
 * Save a blob letting the user pick the name AND the folder, via the File System
 * Access API (Chrome/Edge). Falls back to a normal download when unsupported.
 * Returns false only if the user cancelled the picker.
 */
export async function saveBlobAs(blob, filename) {
  if (typeof window !== 'undefined' && window.showSaveFilePicker) {
    const ext = (filename.split('.').pop() || '').toLowerCase();
    try {
      const handle = await window.showSaveFilePicker({
        suggestedName: filename,
        types: MIME[ext] ? [{ description: ext.toUpperCase(), accept: { [MIME[ext]]: [`.${ext}`] } }] : undefined,
      });
      const w = await handle.createWritable();
      await w.write(blob);
      await w.close();
      return true;
    } catch (e) {
      if (e && e.name === 'AbortError') return false; // cancelled — don't also download
      // any other error: fall through to a normal download
    }
  }
  downloadBlob(blob, filename);
  return true;
}

/**
 * Save several files into a folder the user picks (for stems). Falls back to
 * individual downloads when the directory picker is unavailable.
 */
export async function saveFilesToDir(files) {
  if (typeof window !== 'undefined' && window.showDirectoryPicker) {
    try {
      const dir = await window.showDirectoryPicker();
      for (const f of files) {
        // eslint-disable-next-line no-await-in-loop
        const h = await dir.getFileHandle(f.name, { create: true });
        // eslint-disable-next-line no-await-in-loop
        const w = await h.createWritable();
        // eslint-disable-next-line no-await-in-loop
        await w.write(f.blob);
        // eslint-disable-next-line no-await-in-loop
        await w.close();
      }
      return true;
    } catch (e) {
      if (e && e.name === 'AbortError') return false;
    }
  }
  files.forEach((f, i) => setTimeout(() => downloadBlob(f.blob, f.name), i * 400));
  return true;
}

/** Renders one WAV per mixer insert that has channels routed to it. */
export async function renderStems(project, buffers, opts = {}) {
  const groups = project.inserts
    .map((ins) => ({ ins, chans: project.channels.filter((c) => c.insert === ins.id) }))
    .filter((g) => g.chans.length);
  const out = [];
  for (const g of groups) {
    const soloed = {
      ...project,
      channels: project.channels.map((c) => ({ ...c, mute: c.insert !== g.ins.id, solo: false })),
    };
    // eslint-disable-next-line no-await-in-loop
    const buffer = await renderProject(soloed, buffers, opts);
    out.push({ name: g.ins.name, buffer });
    if (typeof opts.onStem === 'function') opts.onStem(out.length, groups.length);
  }
  return out;
}
