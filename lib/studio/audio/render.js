...
import { PPQ, clamp } from '../constants';
...
export async function renderProject(project, buffers, opts = {}) {
  ...
  for (let r = 0; r < repeats; r++) {
    for (const [tick, events] of map) {
      const time = (tick + r * length) * spt;
      for (const ev of events) {
        const ch = project.channels.find((c) => c.id === ev.channelId);
        if (!ch || ch.mute || (anySolo && !ch.solo)) continue;
        const note = ev.note;
        const dur = Math.max(0.01, note.d * spt);
        const voice = playNote(ctx, graph.channelInput(ch.id), ch, {
          time, key: note.k, vel: clamp(note.v == null ? 0.8 : note.v, 0.01, 1), dur,
        }, buffers);
