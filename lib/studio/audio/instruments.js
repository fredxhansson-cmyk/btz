...
const osc3 = {
  id: 'osc3', name: '3xFLOW', cat: 'Synth', poly: true, color: '#748ffc',
  params: {
...
const sampler = {
  ...
  play(ctx, out, ev, p, buffer) {
    if (!buffer) return null;
    const t = ev.time;
    const src = ctx.createBufferSource();
    src.buffer = buffer;
    if (p.reverse >= 0.5) {
      src.playbackRate.value = 1; // reversal handled at load time
    }
    src.playbackRate.value = Math.pow(2, (ev.key - 60 + p.pitch) / 12);
    const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = clamp(p.cutoff, 30, 19000);
    const amp = ctx.createGain(); amp.gain.value = 0.0001;
    src.connect(lp); lp.connect(amp); amp.connect(out);
    const offset = clamp(p.start, 0, 0.99) * buffer.duration;
    src.start(t, offset);
    const env = { attack: p.attack, decay: p.decay, sustain: p.sustain, release: p.release };
    const end = applyADSR(amp.gain, t, ev.vel, env, ev.dur);
    if (isFinite(end)) stopAll([src], end);
    else src.stop(t + buffer.duration / src.playbackRate.value + 0.1);
    return {
      release(rt) {
...
export function playNote(ctx, out, channel, ev, buffers) {
  ...
  try {
    return inst.play(ctx, out, ev, p, buffers && buffers[channel.id]);
  } catch (e) {
