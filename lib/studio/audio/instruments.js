...
function shaper(ctx, amount) {
  ...
  // Oversampling runs upsample/downsample FIR filters on every render block for
  // the whole life of the node — a heavy, always-on cost that piles up with
  // polyphony during an offline bounce. Only pay it when the drive is strong
  // enough for aliasing to matter; light drive sounds identical without it.
  ws.oversample = amount > 0.45 ? '2x' : 'none';
  return ws;
}
...
const kick = {
  id: 'kick', name: 'FlowKick', cat: 'Drums', poly: false, color: '#ff746e',
  params: {
  ...
  play(ctx, out, ev, p) {
    ...
    // Beater "knock": a short mid-frequency body so the kick reads as a punch on
    // phones and laptops, not just felt sub. This is what makes a synth kick
    // sound like a real one instead of a test tone.
    if (p.knock > 0.001) {
      const kf = clamp(base * 3.4, 90, 520);
      const k = ctx.createOscillator();
      k.type = 'triangle';
      k.frequency.setValueAtTime(kf * 1.6, t);
      k.frequency.exponentialRampToValueAtTime(kf, t + 0.02);
      const kg = percEnv(ctx, t, ev.vel * p.knock * 0.75, 0.035, 0.001);
      // gentle saturation on the knock adds harmonics = more audible cut
      const ks = shaper(ctx, 0.3) || ctx.createGain();
      k.connect(kg); kg.connect(ks); ks.connect(head);
...
const snare = {
  id: 'snare', name: 'FlowSnare', cat: 'Drums', poly: false, color: '#ff746e',
  params: {
...
const clap = {
  id: 'clap', name: 'FlowClap', cat: 'Drums', poly: false, color: '#ff746e',
  params: {
...
const hat = {
  id: 'hat', name: 'FlowHat', cat: 'Drums', poly: false, color: '#ff746e',
  params: {
  ...
  play(ctx, out, ev, p) {
    ...
    // Long, metallic hits (cymbals: high decay) read quieter than a punchy
    // closed hat because their energy is spread over time and up high. Scale
    // the peak up with decay so cymbals sit level with the rest of the kit,
    // while short closed hats (decay ~0.07) are left unchanged.
    const boost = 1 + clamp((p.decay || 0.1) - 0.25, 0, 2) * 0.45;
    const g = percEnv(ctx, t, Math.min(1, ev.vel * 0.6 * boost), p.decay, 0.001);
...
const tom = {
  id: 'tom', name: 'FlowTom', cat: 'Drums', poly: false, color: '#ff746e',
  params: {
...
const perc = {
  id: 'perc', name: 'FlowPerc', cat: 'Drums', poly: false, color: '#ff746e',
  params: {
...
const osc3 = {
  id: 'osc3', name: 'Triple Osc', cat: 'Synth', poly: true, color: '#36b2ff',
  params: {
  ...
  play(ctx, out, ev, p) {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const o = ctx.createOscillator();
      o.type = p[`wave${i}`];
      // 808 / bass slide: glide the pitch in from the previous note.
      const target = midiToFreq(ev.key);
      if (ev.glideFrom != null && p.glide > 0) {
        o.frequency.setValueAtTime(Math.max(20, midiToFreq(ev.glideFrom)), t);
        o.frequency.exponentialRampToValueAtTime(Math.max(20, target), t + p.glide);
      } else {
...
const fm = {
  id: 'fm', name: 'FlowFM', cat: 'Synth', poly: true, color: '#36b2ff',
  params: {
...
const pluck = {
  id: 'pluck', name: 'FlowPluck', cat: 'Synth', poly: true, color: '#36b2ff',
  params: {
...
// Portamento lead — the signature G-funk / talkbox-adjacent whistle synth.
// Each note slides into pitch from an interval offset (glide) and a delayed
// vibrato swells in, which the subtractive engines above can't do.
const glide = {
  id: 'glide', name: 'Glide Lead', cat: 'Synth', poly: true, color: '#36b2ff',
  params: {
    wave: choice('Shape', WAVES, 'sawtooth'),
    glide: f('Glide', 0, 0.4, 0.09, { unit: 's' }),
    range: f('Slide', -12, 12, -5, { step: 1, unit: 'st' }),
    vibRate: f('Vib Rate', 0, 9, 5.2, { unit: 'Hz' }),
    vibDepth: f('Vib Depth', 0, 60, 18, { unit: 'ct' }),
    vibDelay: f('Vib Delay', 0, 1.2, 0.32, { unit: 's' }),
    sub: f('Sub', 0, 1, 0.25),
    cutoff: f('Cutoff', 200, 16000, 4200, { unit: 'Hz', curve: 'log' }),
    res: f('Reso', 0.1, 18, 3),
    envAmt: f('Filter Env', -3, 5, 1.2, { unit: 'oct' }),
    fdecay: f('Filter Dec', 0.02, 3, 0.5, { unit: 's' }),
    attack: f('Attack', 0.001, 2, 0.02, { unit: 's' }),
    decay: f('Decay', 0.01, 3, 0.4, { unit: 's' }),
    sustain: f('Sustain', 0, 1, 0.85),
    release: f('Release', 0.01, 4, 0.3, { unit: 's' }),
    drive: f('Drive', 0, 0.95, 0.15),
  },
  play(ctx, out, ev, p) {
    const t = ev.time;
    const head = driveChain(ctx, out, p.drive);
    const amp = ctx.createGain(); amp.gain.value = 0.0001;
    const filt = ctx.createBiquadFilter();
    filt.type = 'lowpass'; filt.Q.value = p.res;
    const co = clamp(p.cutoff, 60, 18000);
    filt.frequency.setValueAtTime(clamp(co * Math.pow(2, p.envAmt), 60, 19000), t);
    filt.frequency.exponentialRampToValueAtTime(co, t + p.fdecay);
    filt.connect(amp); amp.connect(head);
    ...
    const target = midiToFreq(ev.key);
    const from = midiToFreq(ev.key + p.range);
    const gl = Math.max(0.002, p.glide);

    const osc = ctx.createOscillator();
    osc.type = p.wave;
    osc.frequency.setValueAtTime(from, t);
    osc.frequency.exponentialRampToValueAtTime(target, t + gl);
    const og = ctx.createGain(); og.gain.value = 0.32;
    osc.connect(og); og.connect(filt);
    osc.start(t);
    const oscs = [osc];

    if (p.vibDepth > 0.01 && p.vibRate > 0.01) {
      const lfo = ctx.createOscillator(); lfo.type = 'sine'; lfo.frequency.value = p.vibRate;
      const lg = ctx.createGain();
      lg.gain.setValueAtTime(0.0001, t);
      lg.gain.setValueAtTime(0.0001, t + p.vibDelay);
      lg.gain.linearRampToValueAtTime(p.vibDepth, t + p.vibDelay + 0.2);
    ...
    if (p.sub > 0.001) {
      ...
      so.frequency.setValueAtTime(from / 2, t);
      so.frequency.exponentialRampToValueAtTime(target / 2, t + gl);
      const sg = ctx.createGain(); sg.gain.value = p.sub * 0.35;
      so.connect(sg); sg.connect(filt);
      so.start(t);
      oscs.push(so);
    }
...
// Fender Rhodes-style electric piano: an FM tine (bell-like attack transient
// that decays fast) over a sine body, velocity opening the brightness. Notes
// ring and decay naturally like a struck tine rather than holding a plateau.
const rhodes = {
  id: 'rhodes', name: 'Rhodes EP', cat: 'Keys', poly: true, color: '#85c425',
  params: {
    tine: f('Tine', 0, 1, 0.7),
    ratio: f('Ratio', 0.5, 8, 1, { step: 0.5 }),
    bark: f('Bark', 0.02, 1, 0.14, { unit: 's' }),
    decay: f('Decay', 0.3, 6, 2.4, { unit: 's' }),
    tone: f('Tone', 300, 12000, 4200, { unit: 'Hz', curve: 'log' }),
    velTine: f('Vel>Tine', 0, 1, 0.6),
    release: f('Release', 0.05, 2, 0.35, { unit: 's' }),
    drive: f('Drive', 0, 0.95, 0.08),
  },
  play(ctx, out, ev, p) {
    const t = ev.time;
    const head = driveChain(ctx, out, p.drive);
    ...
    lp.frequency.value = clamp(p.tone * (0.6 + 0.6 * ev.vel), 300, 14000);
    ...
    const idx = p.tine * (0.4 + p.velTine * ev.vel);
    const peakIdx = freq * (3 + idx * 9);
    const mg = ctx.createGain();
    mg.gain.setValueAtTime(peakIdx, t);
    mg.gain.exponentialRampToValueAtTime(Math.max(0.01, peakIdx * 0.05), t + p.bark);
    mod.connect(mg); mg.connect(car.frequency);
    car.connect(lp); lp.connect(amp); amp.connect(head);
    car.start(t); mod.start(t);
    amp.gain.setValueAtTime(0.0001, t);
    amp.gain.linearRampToValueAtTime(ev.vel * 0.85, t + 0.004);
    amp.gain.exponentialRampToValueAtTime(0.0001, t + p.decay);
    const natural = t + p.decay + 0.1;
    stopAll([car, mod], natural);
    return {
      release(rt) {
        const done = releaseADSR(amp.gain, rt, p.release, ev.vel * 0.25);
        stopAll([car, mod], Math.min(natural, done));
      },
    };
  },
};
...
const sampler = {
  id: 'sampler', name: 'FlowSampler', cat: 'Sampler', poly: true, color: '#85c425',
  needsSample: true,
  params: {
    start: f('Start', 0, 1, 0),
    end: f('End', 0, 1, 1),
  ...
  play(ctx, out, ev, p, sample) {
    ...
    src.buffer = buffer;
    src.playbackRate.value = Math.pow(2, (ev.key - (sample.root == null ? 60 : sample.root) + p.pitch) / 12);
...
export const INSTRUMENTS = { kick, snare, clap, hat, tom, perc, osc3, fm, pluck, glide, rhodes, sampler };
...
// Pick the multisample zone whose recorded root is closest to the played note.
function pickZone(zones, key) {
  let best = null;
  let bestDist = Infinity;
  for (const z of zones) {
    const d = Math.abs((z.root == null ? 60 : z.root) - key);
...
export function playNote(ctx, out, channel, ev, buffers) {
  ...
  let sample = buffers && buffers[channel.sampleId || channel.id];
  if (buffers && channel.zones && channel.zones.length) {
    const z = pickZone(channel.zones, ev.key);
    const buf = z && buffers[z.sampleId];
    if (buf) sample = { ...buf, root: z.root == null ? 60 : z.root };
  }
  try {
    return inst.play(ctx, out, ev, p, sample);
  } catch (e) {
