...
export class MixerGraph {
  ...
  // Insert a third-party WAM plugin's AudioNode into the master chain
  // (masterVol -> [wam] -> limiter). One master-FX slot for the foundation.
  insertMasterFx(node) {
    try { this.masterVol.disconnect(); } catch (e) { /* noop */ }
    try { if (this.masterFx) this.masterFx.disconnect(); } catch (e) { /* noop */ }
    this.masterVol.connect(node);
