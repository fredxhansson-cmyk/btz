...
// Three-band compressor for mastering: splits low / mid / high, compresses each
// independently and sums them back. Native nodes only, so it runs live and in
// the offline bounce. Great for glueing a mix without pumping the whole thing.
const multiband = {
  id: 'multiband', name: 'Multiband Comp',
  params: {
    xLow: f('Low/Mid split', 80, 500, 200, { unit: 'Hz', curve: 'log' }),
    xHigh: f('Mid/High split', 1500, 9000, 3000, { unit: 'Hz', curve: 'log' }),
    lowThr: f('Low Thr', -60, 0, -24, { unit: 'dB' }),
    midThr: f('Mid Thr', -60, 0, -22, { unit: 'dB' }),
    highThr: f('High Thr', -60, 0, -24, { unit: 'dB' }),
    ratio: f('Ratio', 1, 12, 3),
    attack: f('Attack', 0.001, 0.2, 0.02, { unit: 's' }),
    release: f('Release', 0.02, 0.6, 0.2, { unit: 's' }),
    lowGain: f('Low Gain', 0, 2, 1),
    midGain: f('Mid Gain', 0, 2, 1),
    highGain: f('High Gain', 0, 2, 1),
    out: f('Output', 0, 2, 1),
  },
  create(ctx) {
    const input = ctx.createGain();
    const output = ctx.createGain();
    const Q = 0.707; // Butterworth per stage; two stages ≈ 24 dB/oct
    const mk = (type) => { const b = ctx.createBiquadFilter(); b.type = type; b.Q.value = Q; return b; };
    // Low band: two cascaded low-passes.
    const lp1 = mk('lowpass'); const lp2 = mk('lowpass');
    // High band: two cascaded high-passes.
    const hp1 = mk('highpass'); const hp2 = mk('highpass');
    // Mid band: high-pass at the low split, low-pass at the high split.
    const mhp = mk('highpass'); const mlp = mk('lowpass');
    const lc = ctx.createDynamicsCompressor();
    const mc = ctx.createDynamicsCompressor();
    const hc = ctx.createDynamicsCompressor();
    const lg = ctx.createGain(); const mg = ctx.createGain(); const hg = ctx.createGain();
    input.connect(lp1); lp1.connect(lp2); lp2.connect(lc); lc.connect(lg); lg.connect(output);
    input.connect(mhp); mhp.connect(mlp); mlp.connect(mc); mc.connect(mg); mg.connect(output);
    input.connect(hp1); hp1.connect(hp2); hp2.connect(hc); hc.connect(hg); hg.connect(output);
    return {
      input, output,
      update(p) {
        lp1.frequency.value = p.xLow; lp2.frequency.value = p.xLow;
        hp1.frequency.value = p.xHigh; hp2.frequency.value = p.xHigh;
        mhp.frequency.value = p.xLow; mlp.frequency.value = p.xHigh;
        for (const c of [lc, mc, hc]) { c.ratio.value = p.ratio; c.attack.value = p.attack; c.release.value = p.release; c.knee.value = 6; }
        lc.threshold.value = p.lowThr; mc.threshold.value = p.midThr; hc.threshold.value = p.highThr;
        lg.gain.value = p.lowGain; mg.gain.value = p.midGain; hg.gain.value = p.highGain;
...
// Dynamic EQ on one band: the band is split out with complementary notch +
// band-pass filters and the band-pass copy is compressed, so the band is only
// tamed (or lifted) when it gets loud. De-esser / harshness-tamer that also
// works in the offline bounce (native nodes, no worklet).
const dyneq = {
  id: 'dyneq', name: 'Dynamic EQ',
  params: {
    freq: f('Freq', 40, 16000, 3000, { unit: 'Hz', curve: 'log' }),
    q: f('Q / width', 0.3, 8, 1.5),
    threshold: f('Threshold', -60, 0, -28, { unit: 'dB' }),
    ratio: f('Ratio', 1, 12, 3),
    attack: f('Attack', 0.001, 0.2, 0.01, { unit: 's' }),
    release: f('Release', 0.02, 0.6, 0.15, { unit: 's' }),
    gain: f('Band gain', 0, 2, 1),
  },
  create(ctx) {
    const input = ctx.createGain();
    const output = ctx.createGain();
    ...
    const c = ctx.createDynamicsCompressor();
    const g = ctx.createGain();
    // Main signal minus the band, plus the dynamically-processed band.
    input.connect(notch); notch.connect(output);
    input.connect(bp); bp.connect(c); c.connect(g); g.connect(output);
    return {
      input, output,
      update(p) {
        notch.frequency.value = p.freq; notch.Q.value = p.q;
        bp.frequency.value = p.freq; bp.Q.value = p.q;
        c.threshold.value = p.threshold; c.ratio.value = p.ratio;
        c.attack.value = p.attack; c.release.value = p.release; c.knee.value = 6;
        g.gain.value = p.gain;
      },
    };
  },
...
export const EFFECTS = { filter, delay, reverb, chorus, phaser, dist, bitcrush, eq3, comp, multiband, dyneq, duck, widener };
