...
export function projectReducer(project, action) {
  switch (action.type) {
    ...
    case 'midi.import': {
      ...
      const pattern = makePattern(action.name || 'MIDI', clamp(action.bars || 1, 1, 64), COLORS[project.patterns.length % COLORS.length]);
      ...
      const ensureRole = (role) => {
        ...
        const ch = makeChannel(spec[0], info.name, {
          color: COLORS[spec[2] % COLORS.length],
          role,
          choke: info.choke,
          insert: insert ? insert.id : null,
          params: spec[1],
