...
export function projectReducer(project, action) {
  switch (action.type) {
    ...
    case 'channels.reset': {
      // Remove every track and start over with a few empty channels.
      const specs = [
        { inst: 'kick', name: 'Kick', role: 'kick', vol: 0.95 },
        { inst: 'snare', name: 'Snare', role: 'snare', vol: 0.55 },
        { inst: 'clap', name: 'Clap', role: 'clap', vol: 0.6 },
        { inst: 'hat', name: 'Closed Hat', role: 'chat', vol: 0.4, choke: 1 },
      ];
      const channels = specs.map((sp, i) => {
        const insert = project.inserts[i % project.inserts.length];
        return makeChannel(sp.inst, sp.name, {
          color: COLORS[i % COLORS.length],
          insert: insert ? insert.id : null,
          vol: sp.vol, role: sp.role, choke: sp.choke,
        });
      });
      const patterns = project.patterns.map((p) => ({ ...p, notes: {} }));
      return { ...project, channels, patterns, selectedChannel: channels[0].id };
    }
    case 'channel.update':
      return { ...project, channels: mapById(project.channels, action.id, (c) => ({ ...c, ...action.patch })) };
    // Swap the instrument/sound on an existing channel, keeping its notes, mix
    // (vol/pan/mute/solo/insert) and position. `samples` (optional) are added to
    // the project so a sampled sound can be swapped in too.
    case 'channel.setInstrument': {
      const samples = action.samples && action.samples.length
        ? { ...(project.samples || {}), ...Object.fromEntries(action.samples.map((s) => [s.id, s])) }
        : project.samples;
      return {
        ...project,
        samples,
        channels: mapById(project.channels, action.id, (c) => ({
          ...c,
          inst: action.inst,
          params: { ...defaultParams(action.inst), ...(action.params || {}) },
          sampleId: action.sampleId || null,
          zones: action.zones || null,
    ...
    case 'channel.reorder': {
      ...
      const moved = project.channels.find((c) => c.id === action.id);
      if (!moved) return project;
      const rest = project.channels.filter((c) => c.id !== action.id);
      const at = action.beforeId ? rest.findIndex((c) => c.id === action.beforeId) : rest.length;
      rest.splice(at < 0 ? rest.length : at, 0, moved);
      return { ...project, channels: rest };
    }
    case 'select.channel':
    ...
    case 'pattern.clearAll':
      return { ...project, patterns: mapById(project.patterns, action.patternId, (p) => ({ ...p, notes: {} })) };
    ...
    // Adds the given samples and creates one multisampled sampler channel that
    // references them as zones. Used by factory multisample instruments.
    case 'samples.instrument': {
      const samples = { ...(project.samples || {}) };
      for (const s of action.samples) samples[s.id] = s;
      const insert = project.inserts[project.channels.length % project.inserts.length];
      const ch = makeChannel('sampler', String(action.name || 'Sampler').slice(0, 22), {
        color: COLORS[project.channels.length % COLORS.length],
        insert: insert ? insert.id : null,
        sampleId: (action.zones[0] || {}).sampleId || null,
        zones: action.zones,
        params: { ...defaultParams('sampler'), ...(action.params || {}) },
      });
      return { ...project, samples, channels: [...project.channels, ch], selectedChannel: ch.id };
    }
    case 'sample.remove': {
    ...
    /* ------------------------------------------------------------ playlist */
    case 'audio.dropClip': {
      // A file dropped on the timeline: create a sampler channel + a pattern
      // that triggers it + a clip at the drop position — all atomically.
      const { sample, track, start, bars } = action;
      const samples = { ...(project.samples || {}), [sample.id]: sample };
      const insert = project.inserts[project.channels.length % project.inserts.length];
      const ch = makeChannel('sampler', String(sample.name || 'Sample').replace(/\.[^.]+$/, '').slice(0, 22), {
        color: COLORS[project.channels.length % COLORS.length],
        insert: insert ? insert.id : null,
        sampleId: sample.id,
      });
      const bt = project.barTicks || BAR_TICKS;
      const nBars = Math.max(1, bars || 1);
      const pat = makePattern(String(sample.name || 'Sample').slice(0, 16), nBars, ch.color, bt);
      pat.notes = { ...(pat.notes || {}), [ch.id]: [makeNote(0, 60, bt * nBars, 1)] };
      const clip = { id: uid('cl'), patternId: pat.id, track: Math.max(0, track || 0), start: Math.max(0, start || 0), length: bt * nBars };
      return {
        ...project,
        samples,
        channels: [...project.channels, ch],
        patterns: [...project.patterns, pat],
        playlist: [...project.playlist, clip],
    ...
    case 'clip.add': {
      ...
      const clip = {
        id: action.id || uid('cl'), patternId: action.patternId, track: action.track, start: Math.max(0, action.start), length: len,
      };
    ...
    case 'automation.record': {
      ...
      const nextLanes = exists
        ...
        : [...lanes, { ...makeLane(action.target, action.label || action.target, action.color || '#36b2ff'), points: write([{ t: 0, v }]) }];
    ...
    case 'track.add':
      return { ...project, arrangeTracks: Math.min(32, (project.arrangeTracks || 10) + 1) };
    case 'track.remove': {
      const cur = project.arrangeTracks || 10;
      const n = Math.max(1, cur - 1);
      if (n === cur) return project;
      const removed = n; // the last lane, index n (0-based)
      const clean = (obj) => { if (!obj) return obj; const o = { ...obj }; delete o[removed]; return o; };
      return {
        ...project,
        arrangeTracks: n,
        playlist: (project.playlist || []).filter((c) => c.track !== removed),
        trackMute: clean(project.trackMute),
        trackSolo: clean(project.trackSolo),
        trackNames: clean(project.trackNames),
      };
    }
    case 'marker.add':
    ...
    case 'tempo.add':
      return {
        ...project,
        tempoMap: [
          ...(project.tempoMap || []).filter((m) => Math.abs(m.tick - action.tick) > 1),
          { id: uid('tm'), tick: Math.max(0, Math.round(action.tick)), bpm: clamp(action.bpm, 20, 300) },
        ].sort((a, b) => a.tick - b.tick),
      };
    case 'tempo.remove':
      return { ...project, tempoMap: (project.tempoMap || []).filter((m) => m.id !== action.id) };
    case 'take.save': {
      const pat = project.patterns.find((p) => p.id === action.patternId);
      if (!pat) return project;
      const notes = (pat.notes && pat.notes[action.channelId]) || [];
      const takes = { ...(pat.takes || {}) };
      const list = takes[action.channelId] || [];
      takes[action.channelId] = [...list, { id: uid('tk'), name: action.name || `Take ${list.length + 1}`, notes: notes.map((n) => ({ ...n })) }];
      return { ...project, patterns: mapById(project.patterns, action.patternId, (p) => ({ ...p, takes })) };
    }
    case 'take.load': {
      const pat = project.patterns.find((p) => p.id === action.patternId);
      const take = pat && ((pat.takes || {})[action.channelId] || []).find((t) => t.id === action.takeId);
      if (!take) return project;
      return { ...project, patterns: mapById(project.patterns, action.patternId, (p) => ({ ...p, notes: { ...(p.notes || {}), [action.channelId]: take.notes.map((n) => ({ ...n })) } })) };
    }
    case 'take.remove': {
      const pat = project.patterns.find((p) => p.id === action.patternId);
      if (!pat) return project;
      const takes = { ...(pat.takes || {}) };
      takes[action.channelId] = (takes[action.channelId] || []).filter((t) => t.id !== action.takeId);
      return { ...project, patterns: mapById(project.patterns, action.patternId, (p) => ({ ...p, takes })) };
    }
    case 'clip.split': {
    ...
    case 'insert.addBus':
      return { ...project, inserts: [...project.inserts, { ...makeInsert(project.inserts.length + 1), name: `Bus ${project.inserts.filter((i) => i.bus).length + 1}`, bus: true }] };
    case 'select.insert':
...
export function historyReducer(state, action) {
  ...
  // Remote (collaborator) edits must never enter your local undo stack.
  if (NO_HISTORY.has(action.type) || action.remote) return { ...state, project };
