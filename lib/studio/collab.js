...
export function getStatus() {
  ...
  let peers = [];
  if (a) peers = [...a.getStates().entries()].map(([id, s]) => ({ id, self: id === a.clientID, ...(s && s.user ? s.user : {}) }));
...
export async function joinRoom(roomId, user, onRemote) {
  ...
  state = { doc, provider, room: roomId, awareness, root, Y, self: user, onRemote };
...
export function leaveRoom() {
  ...
  state = { doc: null, provider: null, room: null, awareness: null, root: null, Y: null, self: null, onRemote: null };
  emit();
}
