...
import { onCollab, joinRoom as collabJoin, leaveRoom as collabLeave, pushProject as collabPush, isActive as collabActive } from './collab';
...
export function StudioProvider({ children }) {
  ...
  const joinCollab = useCallback((roomId) => {
    ...
    collabJoin(room, meRef.current, (json) => {
      ...
      dispatch({ type: 'set', project: merged });
    });
  ...
  const value = useMemo(() => ({
    ...
    play, stop, pause, togglePlay, setMode, newProject, loadTemplate, addToArrangement, saveFile, openFile, exportWav, exportAudio, loadSampleFile, separateStemsFile,
    exportStems, exportMidiFile, importMidiFile, midiInputs,
    recordNote, finishRecordedNote, recordAuto, busy,
    detached, popOut, attach,
    collab, joinCollab, leaveCollab, me: meRef.current,
    ...
    newProject, saveFile, openFile, exportWav, exportAudio, addToArrangement, recordNote, finishRecordedNote, busy,
    detached, popOut, attach, collab, joinCollab, leaveCollab,
