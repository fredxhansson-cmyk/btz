...
export function StudioProvider({ children }) {
  ...
  /** Swap the instrument/sound on an existing channel (keeps its notes + mix).
      Works for both synth presets and sampled sounds. */
  const replaceInstrument = useCallback(async (channelId, sound) => {
    if (!channelId || !sound) return;
    if (sound.kind !== 'sample') {
      dispatch({ type: 'channel.setInstrument', id: channelId, inst: sound.inst, params: sound.params || {}, name: sound.name });
      setHint(`Track changed to ${sound.name}.`);
      setUi({ view: 'rack', browserOpen: false });
      return;
    }
    setBusy(`Loading ${sound.name}…`);
    try {
      const ctx = engine.ensureContext();
      if (!ctx) throw new Error('Audio engine not ready.');
      const decodeUrl = async (url) => { const r = await fetch(url); if (!r.ok) throw new Error('Could not load that sample.'); return ctx.decodeAudioData(await r.arrayBuffer()); };
      const toSample = async (buf, name) => {
        const wav = encodeWav(buf); const bytes = new Uint8Array(await wav.arrayBuffer());
        const sample = { id: uid('sm'), name: String(name).slice(0, 22), mime: 'audio/wav', bytes: bytes.length, data: bytesToBase64(bytes) };
        await engine.decodeSample(sample); return sample;
      };
      if (sound.zones && sound.zones.length) {
        const samples = []; const zones = [];
        for (const z of sound.zones) {
          // eslint-disable-next-line no-await-in-loop
          const buf = await decodeUrl(z.url);
          // eslint-disable-next-line no-await-in-loop
          const sample = await toSample(buf, `${sound.name} ${z.root}`);
          samples.push(sample); zones.push({ sampleId: sample.id, root: z.root });
        }
        dispatch({ type: 'channel.setInstrument', id: channelId, inst: 'sampler', samples, zones, params: sound.params || {}, name: sound.name });
      } else {
        const buf = await decodeUrl(sound.url); const sample = await toSample(buf, sound.name);
        dispatch({ type: 'channel.setInstrument', id: channelId, inst: 'sampler', samples: [sample], sampleId: sample.id, params: sound.params || {}, name: sound.name });
      }
      setHint(`Track changed to ${sound.name}.`);
      setUi({ view: 'rack', browserOpen: false });
    } catch (e) {
      setHint(e.message || 'Could not change instrument.');
    } finally {
      setBusy(null);
    }
  }, [engine, dispatch, setBusy, setHint, setUi]);
  ...
  const value = useMemo(() => ({
    ...
    play, stop, pause, togglePlay, setMode, newProject, loadTemplate, addToArrangement, saveFile, openFile, exportWav, exportAudio, renderMasterBlob, loadSampleFile, separateStemsFile, decodeUrlAudio, addBufferChannel, addSampleSound, replaceInstrument, dropAudioOnTimeline,
    ...
    detached, popOut, attach, collab, joinCollab, leaveCollab, addSampleSound, replaceInstrument,
