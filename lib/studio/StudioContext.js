...
import { songLength } from './sequencer';
...
export function StudioProvider({ children }) {
  ...
  const addToArrangement = useCallback(() => {
    ...
    dispatch({ type: 'clip.add', patternId: p.activePattern, track: 0, start: songLength(p) });
    setUi({ view: 'playlist', mode: 'song' });
    setHint(`"${pat ? pat.name : 'Pattern'}" added to the arrangement.`);
  ...
  const value = useMemo(() => ({
    ...
    play, stop, togglePlay, setMode, newProject, loadTemplate, addToArrangement, saveFile, openFile, exportWav, exportAudio, loadSampleFile,
