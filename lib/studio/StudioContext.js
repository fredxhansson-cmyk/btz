...
import { BAR_TICKS } from './constants';
...
export function StudioProvider({ children }) {
  ...
  const recordNote = useCallback((key, vel) => {
    if (!uiRef.current.recording || !engine.playing) return null;
    const p = projectRef.current;
    const tick = Math.round(engine.currentPosition());
    const note = { id: `rec${Math.random().toString(36).slice(2)}`, t: tick, k: key, d: 24, v: vel };
    dispatch({ type: 'note.add', patternId: p.activePattern, channelId: p.selectedChannel, note });
    return note;
  }, [engine]);
  ...
  const finishRecordedNote = useCallback((note) => {
    ...
    const d = Math.max(6, end > note.t ? end - note.t : 24);
    dispatch({ type: 'note.update', patternId: p.activePattern, channelId: p.selectedChannel, id: note.id, patch: { d } });
  }, [engine]);
