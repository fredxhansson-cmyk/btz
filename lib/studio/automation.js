...
const MIX_SPECS = {
  vol: { min: 0, max: 1.2, def: 0.8, label: 'Volym' },
...
export function listTargets(project) {
  const out = [];
  out.push({ id: 'mix|master||vol', group: 'Master', label: 'Master – Volym', spec: { min: 0, max: 1.4, def: 0.75 } });
  for (const ch of project.channels) {
    out.push({ id: `mix|channel|${ch.id}|vol`, group: 'Kanaler', label: `${ch.name} – Volym`, spec: MIX_SPECS.vol });
    out.push({ id: `mix|channel|${ch.id}|pan`, group: 'Kanaler', label: `${ch.name} – Pan`, spec: MIX_SPECS.pan });
  ...
  for (const ins of project.inserts) {
    out.push({ id: `mix|insert|${ins.id}|vol`, group: 'Mixer', label: `${ins.name} – Volym`, spec: MIX_SPECS.vol });
