...
export function exportMidi(project, mode = 'song') {
  ...
  tracks.push(trackChunk(meta, project.name || 'FLOW Studio'));
...
export function parseMidi(bytes) {
  ...
  if (r.str(4) !== 'MThd') throw new Error('Inte en MIDI-fil.');
  r.u32();
  r.u16();
  const ntrks = r.u16();
  const division = r.u16();
  if (division & 0x8000) throw new Error('SMPTE-baserade MIDI-filer stods inte.');
